...
def seed_system_bank_accounts(conn: sqlite3.Connection) -> None:
    ...
    if count:
        return
    # Bank account details are legal and operational data. Never invent an active IBAN.
...
def seed_admin(conn: sqlite3.Connection) -> None:
    explicit_seed = bool(os.environ.get("ADMIN_TC") or os.environ.get("ADMIN_PASSWORD"))
    existing_admin = conn.execute("SELECT id FROM users WHERE role='admin' LIMIT 1").fetchone()
    if existing_admin and not explicit_seed:
        return
    ...
    conn.commit()
    if generated:
        bootstrap = DATA_DIR / "bootstrap_admin.txt"
        bootstrap.write_text(f"TC={admin_tc}\nPASSWORD={password}\nCREATED_AT={iso_time()}\n", encoding="utf-8")
        print(f"Local bootstrap admin created: {bootstrap}")
...
def seed_test_user(conn: sqlite3.Connection) -> None:
    ...
    cash = max(0.0, env_float("TEST_USER_CASH", 250000.0))
    credit = max(0.0, env_float("TEST_USER_CREDIT", 0.0))
    if existing:
        updates = [full_name, email, phone, city, district, now(), existing["id"]]
        password_sql = ""
        if password:
            salt, digest = hash_password(password)
            password_sql = "password_salt=?, password_hash=?,"
        ...
        symbol = re.sub(r"[^A-Z0-9]", "", os.environ.get("TEST_USER_SYMBOL", "AKBNK").upper())
        quantity = max(0, env_int("TEST_USER_POSITION_QTY", 10))
        price = max(0.0, env_float("TEST_USER_POSITION_PRICE", 67.65))
        if symbol and quantity and price:
            conn.execute(
                "INSERT OR IGNORE INTO positions (user_id, symbol, quantity, avg_price, updated_at) VALUES (?, ?, ?, ?, ?)",
                (existing["id"], symbol, quantity, price, now()),
            )
    ...
    cur = conn.execute(
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'user', 'approved', 'test_account', 1, ?, ?)
        """,
    ...
    if cash:
        write_transaction(conn, user_id, "seed_cash", cash, 0, cash, note="Render test kullanici bakiyesi")
    symbol = re.sub(r"[^A-Z0-9]", "", os.environ.get("TEST_USER_SYMBOL", "AKBNK").upper())
    quantity = max(0, env_int("TEST_USER_POSITION_QTY", 10))
    price = max(0.0, env_float("TEST_USER_POSITION_PRICE", 67.65))
    if symbol and quantity and price:
        conn.execute(
            "INSERT INTO positions (user_id, symbol, quantity, avg_price, updated_at) VALUES (?, ?, ?, ?, ?)",
            (user_id, symbol, quantity, price, created),
        )
        write_transaction(
            conn,
            user_id,
            ...
            quantity=quantity,
            price=price,
            note="Render test kullanici pozisyonu",
        )
...
class AppHandler(BaseHTTPRequestHandler):
    ...
    def api_login(self) -> None:
        ...
        with connect_db() as conn:
            ...
            if False and int(user["two_factor_enabled"] or 0) and not verify_totp(str(user["two_factor_secret"] or ""), str(payload.get("otp", ""))):
            ...
            if self.headers.get("X-Forwarded-Proto") == "https":
                cookie += "; Secure"
            self.json_response({"user": public_user(dict(user))}, extra_headers={"Set-Cookie": cookie})
    ...
    def api_register(self) -> None:
        ...
        try:
            ...
            suitability_score = -1
        if not full_name or not valid_turkish_identity_number(tc) or not password_is_strong(password) or not phone or (email and not re.fullmatch(r"[^@\s]+@[^@\s]+\.[^@\s]+", email)):
        ...
        with connect_db() as conn:
            ...
            self.json_response({
                ...
                "branding": {
                    ...
                    "tagline": settings.get("brand_tagline", "Yatırımın dijital hali"),
                    "primary": settings.get("ui_primary_color", "#5b52e0"),
                    "accent": settings.get("ui_accent_color", "#00a96b"),
                    "danger": settings.get("ui_danger_color", "#ef3340"),
                    "font": settings.get("ui_font_family", "Inter"),
                    "radius": settings.get("ui_radius", "18"),
                    "support_email": settings.get("content_support_email", settings.get("official_email", "")),
                    "support_phone": settings.get("content_support_phone", settings.get("official_phone", "")),
                },
    ...
    def api_create_money_request(self) -> None:
        ...
        with connect_db() as conn:
            ...
            if request_type == "deposit":
                ...
                if not receipt:
                    raise HttpError(400, "Para yatırma talebi için dekont zorunludur")
...
def order_rows(conn: sqlite3.Connection, where: str, params: tuple) -> list[dict]:
    ...
    return [decorate_order(dict(row)) for row in rows]
